//! This module contains the bindings of the openslide library and its rust wrappers. The api of
//! the wrappers should be consistant with the C API of OpenSlide
//! `https://openslide.org/api/openslide_8h.html`
//!
//! For a more rust convenient api, see the convenience module
//!
...
extern {
    ...
    fn openslide_get_level0_dimensions(osr: *const OpenSlideT,
                                       w: *mut libc::int64_t,
                                       h: *mut libc::int64_t) -> libc::c_void;

    fn openslide_get_level_dimensions(osr: *const OpenSlideT,
                                      level: libc::int32_t,
                                      w: *mut libc::int64_t,
                                      h: *mut libc::int64_t) -> libc::c_void;

    fn openslide_get_level_downsample(osr: *const OpenSlideT,
                                      level: libc::int32_t) ->  libc::c_double;

    fn openslide_get_best_level_for_downsample(slide: *const OpenSlideT,
                                               downsample_factor: libc::c_double) -> libc::int32_t;

    fn openslide_read_region(osr: *const OpenSlideT,
                             dest: *mut libc::uint32_t,
                             x: libc::int64_t,
                             y: libc::int64_t,
                             level: libc::int32_t,
                             w: libc::int64_t,
                             h: libc::int64_t) -> libc::c_void;
    ...
    fn openslide_get_property_value(osr: *const OpenSlideT,
...
pub fn close(osr: *const OpenSlideT) -> Result<(), Error> {
    unsafe {
        openslide_close(osr);
    }
    Ok(())
...
pub fn get_best_level_for_downsample(osr: *const OpenSlideT, downsample: f64) -> Result<i32, Error> {
    let level = unsafe {
...
pub fn read_region(osr: *const OpenSlideT,
                   x: i64,
                   y: i64,
                   level: i32,
                   w: i64,
                   h: i64) -> Result<Vec<u32>, Error> {
...
pub fn get_error(osr: *const OpenSlideT) -> Result<String, Error> {
    let msg = unsafe {
